...
5. `lyndon/patch-connector-routing` (`bed91e0e40d1`) — GitHub default, Native2 available on every normal Web turn, keyboard/multi-pill selection, and Native2-before-GitHub dual-app order. Compaction remains tool-free. Upstream owns stale connector-catalog refresh/reverification and 2.1.10 approval-card handling; this patch layers routing policy and selection behavior on that path.
