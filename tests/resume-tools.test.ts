...
function resumedRequest(options: { tools?: CodexTool[]; explicitWireTools?: unknown[] } = {}): CodexParsedRequest {
  return {
    modelId: "gpt-5.6-sol",
    stream: true,
    context: {
...
test("deferred tool-search additions merge with an omitted persisted base registry", () => {
  ...
  const resumed = new ChatGptThreadEnvironmentStore(path).resolve(resumedRequest({ tools: [deferred] }));
